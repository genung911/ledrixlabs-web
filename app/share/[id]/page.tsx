...
function HomeTab({ record, anomalies, projects, reminders, repairs, onTabChange, access, shareId, onUnlock, onAsk }: {
  ...
  const pillars: [Tab, IconName, string, number | string][] = [
    ['report', 'docs', 'Reports', ''],
    ['findings', 'findings', 'Punch List', anomalies.filter(a => !['wear', 'good'].includes(priorityOf(a).key)).length],
...
function FindingsTab({ anomalies, record, shareId, floorPlan }: { anomalies: Anomaly[]; record: HomeRecord; shareId: string; floorPlan?: FloorPlanRow | null }) {
  ...
  return (
    <div style={{ padding: '16px 16px 0' }}>
      ...
      <div style={{ fontFamily: SERIF, color: TEXT, fontSize: 22, fontWeight: 600, marginBottom: 16 }}>Punch List</div>
